...
public class InitiationforWeibo {
	...
	/**
	 * Mining data with another application. In details, we just change the
	 * consumer_key and consumer_secret in Class Weibo.
	 */
	...
	public static Weibo intiation() {
		...
		weibo.setToken("b84ea958dc97658992bcfb985015337c",
				"a5d965b3919883c66d03802e5a476715");
		return weibo;
	}

	/**
	 * @param processing
	 *            initiate the WeiboList with weibos, the number of weibos is
	 *            WeiboNumberMax
	 */
	public void initiations(Processing processing) {
		/*
		 * If the information of Weibo Account is initiated before, return the
		 * results of afterinitiations, otherwise, fetch the information of
		 * Weibo Account by httpOauth.
		 */
		...
		for (int i = 0; i < WeiboNumberMax; i++) {
			try {
				...
				RequestToken requestToken = WeiboList.get(i)
				...
				System.out.println(WeiboList.get(i).showUser("David_Scoot")
						.toString());
				ExportDataToMongo export = new ExportDataToMongo();
				export.ExportAccountInformation(processing, weiboAccount[i],
						weiboPassword[i], weiboToken[i], weiboTokenSecret[i]);
			} catch (IOException e) {
	...
	/**
	 * @param processing
	 * @return whether the account information is fetched by HttpOauth at the
	 *         first time if the return is true, initiate weibo accounts from
	 *         the database
	 */
	public boolean afterinitiations(Processing processing) {
		...
		ArrayList<String> AccountInfo = imports.importAccountInfomation(
				processing, WeiboNumberMax);
