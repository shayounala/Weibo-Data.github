...
public class Process {
	...
	public static void main(String[] args) {
		...
		try {
			...
			DBCollection userinformationfilter = db
					.getCollection("UserInformationFilter");
			userinformationfilter.drop();

			ArrayList<Integer> uniqueusers = (ArrayList<Integer>) userinformation
					.distinct("ID");
			System.out.println("finish the unique reading: "
					+ uniqueusers.size());
			// System.exit(0);
			...
			for (int i = 0; i < uniqueusers.size(); i++) {
				...
				object = (BasicDBObject) userinformation
						.findOne(new BasicDBObject("ID", uniqueusers.get(i)));
			...
			System.out.println("Number of Unique Users: "
					+ userinformationfilter.distinct("ID").size());
