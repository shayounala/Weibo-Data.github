...
public class TransferDatatoMongo {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			/*
			 * Read the old data of Next ID and the length of the Unique User
			 * IDs
			 */
			...
			UniqueUsersIDList.add(1774839495);// Important, in the previous
												// results of the Unique User
												// IDs, we
			// forget to record the root node id, here we add the root node to
			// the results.
			...
			for (int i = 0; i < UniqueUsersIDList.size(); i++) {
				for (int j = i + 1; j < UniqueUsersIDList.size(); j++) {
					if (UniqueUsersIDList.get(i) == UniqueUsersIDList.get(j)) {
						System.out
								.println("False ID: Same ID in the getFollowersIDs");
						System.exit(0);
					}
				}
			}
			...
			resultsread = new DataInputStream(
					new FileInputStream("results.txt"));
			loop: for (int i = 0; i < nextID - 1; i++) {
				boolean flag = false;// mark if the followers of ID i is
										// finished reading
				while (!flag) {
					int readin = 0;
					try {
			...
			resultsread = new DataInputStream(new FileInputStream(
					"results1.txt"));
			loop: for (int i = 0; i < nextID - 1; i++) {
				boolean flag = false;// mark if the followers of ID i is
										// finished reading
			 ...
			 * UsersIDList.add(followersID.get(0)); for(int
			 * i=0;i<followersID.size();i++){ int id = followersID.get(i); if(id
			 * == -1 && (i+1)<followersID.size()){
			 * if(!UsersIDList.contains(followersID.get(i+1))){
			 * UsersIDList.add(followersID.get(i+1)); } } }
			 * System.out.println(UsersIDList.size()); for(int
			 * i=0;i<UsersIDList.size();i++){ if(UsersIDList.get(i).intValue()
			 * != UniqueUsersIDList.get(i).intValue()){
			 * //System.out.println("Reading the Users'followers is wrong");
			 * System
			 * .out.println("UniqueUserID: "+UniqueUsersIDList.get(i)+"UserID: "
			 * +UsersIDList.get(i)+"Difference: "+(UniqueUsersIDList.get(i)-
			 ...
			 * query.put("Next ID for Mining followers ID", new
			 * BasicDBObject(QueryOperators.EXISTS, true));
			 * System.out.println(NextIDsCollection
			 * .find(query).toArray().get(0).
			 * get("Next ID for Mining followers ID")); int nextid =
			 * UniqueUsersIDList
			 * .get(NextIDsCollection.find(query).toArray().get(
			 * 0).get("Next ID for Mining followers ID").hashCode());
			 * NextIDsCollection.update(query, new
			 * BasicDBObject("Next ID for Mining followers ID",nextid));
			 * System.out
			 * .println(NextIDsCollection.find(query).toArray().get(0).
			 * get("Next ID for Mining followers ID")); System.exit(0)
			 */;
			...
			for (int i = 0; i < UniqueUsersIDList.size(); i++) {
				UniqueUserIDsCollection.insert(new BasicDBObject("Number", i)
						.append("ID", UniqueUsersIDList.get(i).intValue()));
				// UniqueUserIDsCollection.insert(new
				// BasicDBObject().append("Unique User ID",
				// UniqueUsersIDList.get(i).intValue()));
			}
			// System.exit(0);
			...
			System.out.println(UserInformationCollection.find(
					new BasicDBObject("ID", new BasicDBObject(
