...
public class FilterToLocal {
	...
	public static void main(String[] args) {
		try {
			...
			//OutputFilterdbInformation(dbFilter, dbFiltered, "UserInformation1");
	...
	private static void testDataBase(DB dbFiltered, DB weibodb) {
		...
		/*weibodb.getCollection("RegularUserInformation").drop();
		DBCollection collection = dbFiltered.getCollection("UserInformation1");
		DBCollection collection1 = weibodb.getCollection("FilterUserInformation");
		
		DBCursor cursor = collection.find();
		
		for(int i=0;cursor.hasNext();i++){
			collection1.insert(cursor.next());
			System.out.println(i);
		}*/
		
		System.out.println(weibodb.getCollection("UserInformation").count());
		System.out.println(weibodb.getCollection("FilterUserInformation").count());
		
		
	}

	@SuppressWarnings("unchecked")
	private static void OutputFilterdbInformation(DB dbFilter, DB dbFiltered, String CollectionName) {
		// TODO Auto-generated method stub
		...
		DBCursor cursor = collection1.find(new BasicDBObject("User IDs", new BasicDBObject(QueryOperators.EXISTS, true))).limit(5200);
	...
	@SuppressWarnings("unchecked")
	private static void FilterFollowersForLocal(DB dbFilter, String CollectionName) {
		// TODO Auto-generated method stub
		DBCollection collection = dbFilter.getCollection(CollectionName);
	...
	private static void WriteToLocal(DB dbLocal, String LocalCollectionName,
		...
		System.out.println("Count of UserInformation in Filterdb: "+collection.count());
	...
	private static void FromServerToLocal(DB dbServer,
		...
		for (int i = 0; i < cursor.size(); i++) {
			...
			followers = (ArrayList<Long>) information
					.get("Followers ID");
